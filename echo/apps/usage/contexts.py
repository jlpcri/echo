...
def projects_context(sort=None):
    ...
    for p in project_list:
        project_pass = Action.objects.filter(scope__project=p, type=Action.TESTER_PASS_SLOT).count()
        project_fail = Action.objects.filter(scope__project=p, type=Action.TESTER_FAIL_SLOT).count()
        project_total = project_pass + project_fail
        project_auto_pass = Action.objects.filter(scope__project=p, type=Action.AUTO_PASS_SLOT).count()
        project_auto_fail = Action.objects.filter(scope__project=p, type=Action.AUTO_FAIL_SLOT).count()
        project_auto_missing = Action.objects.filter(scope__project=p, type=Action.AUTO_MISSING_SLOT).count()
...
def users_context(sort=None):
    ...
    for u in user_list:
        users.append(
            {
                'user': u,
                'total': Action.objects.filter(actor=u, type=Action.TESTER_PASS_SLOT).count() + Action.objects.filter(actor=u, type=Action.TESTER_FAIL_SLOT).count(),
                'passed': Action.objects.filter(actor=u, type=Action.TESTER_PASS_SLOT).count(),
                'defective': Action.objects.filter(actor=u, type=Action.TESTER_FAIL_SLOT).count(),
                'reports': Action.objects.filter(actor=u, type=Action.REPORT_GENERATION).count(),
                'uploads': Action.objects.filter(actor=u, type=Action.UPLOAD_VUID).count(),
                'projects': Action.objects.filter(actor=u, type=Action.CREATE_PROJECT).count()
            }
        )
